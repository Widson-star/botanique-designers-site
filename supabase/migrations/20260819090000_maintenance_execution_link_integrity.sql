-- BOTANIQUE DESIGNERS — Maintenance execution link integrity.
--
-- Three corrections to the Site-aware Daily Site Record:
--
-- 1. A project-less Daily Site Record is legitimate ONLY where the Site has an
--    ACTIVE Maintenance relationship. An Ongoing Botanique Project by itself is
--    NOT authority to drop Project context — ordinary implementation work keeps
--    its Project.
--
-- 2. Site + date does NOT uniquely identify a Maintenance field record, because
--    one Site may legitimately hold several Projects and therefore several
--    records on one date. daily_site_entries.maintenance_visit_id makes the
--    association a durable fact instead of a same-date guess. The Maintenance
--    visit remains the plan/occurrence; the Daily Site Record remains the
--    execution record; this only links the two.
--
-- 3. Authority follows the record's nature: an explicitly Maintenance-linked
--    record answers to Maintenance Site authority, while an ordinary Project
--    record keeps unchanged Project authority. This deliberately does NOT grant
--    a manager access to unrelated ordinary Project records at the same Site.

-- --------------------------------------------------------------- eligibility --

-- "This Site currently runs Maintenance" — the only thing that justifies a
-- field record with no Botanique Project.
create or replace function public.private_site_has_active_maintenance(target_site_id uuid)
returns boolean
...
alter table public.daily_site_entries
  add column maintenance_visit_id uuid null
    references public.maintenance_visits(id) on delete restrict;

-- Structural backfill from a fact the system ALREADY recorded at closure time:
-- maintenance_visits.daily_site_entry_id. Only pairs that are provably coherent
-- (same Site, same date) are linked; anything ambiguous stays null rather than
-- manufacturing history. daily_site_entries carries no triggers, so this writes
-- no event and touches no actor, timestamp, state or version.
update public.daily_site_entries e
set maintenance_visit_id = v.id
from public.maintenance_visits v
join public.maintenance_relationships r on r.id = v.maintenance_relationship_id
where v.daily_site_entry_id = e.id
  and e.maintenance_visit_id is null
  and e.site_id = r.site_id
  and e.work_date = v.scheduled_date;

create index daily_site_entries_maintenance_visit_idx
  on public.daily_site_entries (maintenance_visit_id)
  where maintenance_visit_id is not null;

-- One LIVE field record may claim a given Maintenance visit. Null stays
-- unrestricted, and supersession works because the original is marked
-- superseded (a non-live state) before the replacement is inserted.
create unique index daily_site_entries_one_live_per_maintenance_visit
  on public.daily_site_entries (maintenance_visit_id)
  where maintenance_visit_id is not null
...
drop function if exists public.can_manage_daily_site_record(uuid, uuid);

-- Authority by the record's own nature:
--   linked to a Maintenance visit -> Maintenance Site authority
--   carries a Project             -> unchanged Project authority
--   neither                       -> only where the Site runs Maintenance
...
as $$
  select case
    when target_maintenance_visit_id is not null then public.can_manage_maintenance_site(target_site_id)
    when target_project_id is not null then public.can_manage_daily_site_project(target_project_id)
    else public.private_site_has_active_maintenance(target_site_id)
         and public.can_manage_maintenance_site(target_site_id)
  end
$$;

create policy daily_site_entries_select_authorised on public.daily_site_entries
  for select to authenticated
  using (public.can_manage_daily_site_record(project_id, site_id, maintenance_visit_id));

-- ------------------------------------------------------------- site picker --

-- The picker must state whether Maintenance is actually active, so the UI never
-- infers "maintenance only" merely from the absence of a chosen Project.
drop function if exists public.daily_site_authorised_sites();
create function public.daily_site_authorised_sites()
returns table(
  id uuid, site_name text, location text, county text,
  projects jsonb, has_active_maintenance boolean, maintenance_relationship_id uuid
)
language sql stable security definer set search_path to 'public'
as $$
  select s.id, s.site_name, s.location, s.county,
    ...
    public.private_site_has_active_maintenance(s.id),
    (select r.id from public.maintenance_relationships r
      where r.site_id = s.id and r.status = 'active' limit 1)
  from public.sites s
...
drop function if exists public.create_daily_site_entry_draft_for_site(
  uuid,uuid,date,text,text,text,integer,text,numeric,numeric,text,numeric,numeric,text,text);

create function public.create_daily_site_entry_draft_for_site(
  target_site_id uuid,
  target_project_id uuid,
...
declare
  ...
  visit public.maintenance_visits;
...
begin
  ...
  -- The optional Project must always belong to the same Site, whatever the
  -- record's nature.
  ...
  if target_maintenance_visit_id is not null then
    -- Explicit Maintenance execution: Maintenance authority answers for it, and
    -- Project authority is NOT additionally required just because the record
    -- also carries its originating Project.
    select * into visit from public.maintenance_visits where id = target_maintenance_visit_id for share;
    if not found then
      raise exception 'maintenance visit not found' using errcode = 'no_data_found';
    end if;
    select * into relationship from public.maintenance_relationships
      where id = visit.maintenance_relationship_id for share;
    if relationship.site_id <> site.id then
      raise exception 'that maintenance visit belongs to a different site' using errcode = 'check_violation';
    end if;
    if visit.scheduled_date <> target_work_date then
      raise exception 'the work date must match the scheduled maintenance visit date' using errcode = 'check_violation';
    end if;
    if visit.status <> 'scheduled' then
      raise exception 'only a scheduled maintenance visit can receive a new field record' using errcode = 'check_violation';
    end if;
    if not public.can_manage_maintenance_site(relationship.site_id) then
      raise exception 'not authorised for this maintenance site' using errcode = 'insufficient_privilege';
    end if;
  elsif target_project_id is not null then
    -- Ordinary Project execution keeps its unchanged Project authority + rules.
    ...
    -- Project-less and unlinked: only ever legitimate where Maintenance is live.
    if not public.private_site_has_active_maintenance(site.id) then
      raise exception 'a field record without a Botanique Project requires an active Maintenance relationship at this site' using errcode = 'check_violation';
    end if;
    if not public.can_manage_maintenance_site(site.id) then
      raise exception 'not authorised for this maintenance site' using errcode = 'insufficient_privilege';
    end if;
  end if;
  ...
  insert into public.daily_site_entries (
    site_id, project_id, maintenance_visit_id, work_date, disposition, no_work_reason, reason_detail,
    ...
    site.id, target_project_id, target_maintenance_visit_id, target_work_date, target_disposition,
    plan.out_no_work_reason, plan.out_reason_detail,
...
create or replace function public.create_daily_site_entry_draft(
  target_project_id uuid, target_work_date date, target_disposition text,
...
begin
  ...
  return public.create_daily_site_entry_draft_for_site(
    ...
    target_additional_requested, target_notes, target_evidence_status, null
  );
end;
$$;
...
begin
  ...
  if not public.can_manage_daily_site_record(entry.project_id, entry.site_id, entry.maintenance_visit_id) then
...
-- Supersession must carry Site, Project AND Maintenance-visit identity forward.
...
begin
  ...
  -- Mark the original superseded first so both partial-unique live indexes free up.
  update public.daily_site_entries set
  ...
  insert into public.daily_site_entries (
    ...
    original.site_id, original.project_id, original.maintenance_visit_id, original.work_date,
    target_disposition, plan.out_no_work_reason, plan.out_reason_detail,
    plan.out_worker_count, plan.out_crew_reference, plan.out_rate, plan.out_agreed,
    plan.out_planned_cost, plan.out_work_planned, plan.out_funds_available, plan.out_additional_requested,
...
-- A record explicitly linked to Visit A must never close Visit B, even when the
-- Site and date happen to match.
...
begin
  ...
  if target_daily_site_entry_id is not null then
    ...
    if execution.maintenance_visit_id is not null and execution.maintenance_visit_id <> existing.id then
      raise exception 'This Daily Site Record is the execution truth for a different Maintenance visit' using errcode='22023';
    end if;
...
revoke execute on function public.private_site_has_active_maintenance(uuid) from public, anon;
revoke execute on function public.can_manage_daily_site_record(uuid, uuid, uuid) from public, anon;
revoke execute on function public.daily_site_authorised_sites() from public, anon;
revoke execute on function public.create_daily_site_entry_draft_for_site(uuid,uuid,date,text,text,text,integer,text,numeric,numeric,text,numeric,numeric,text,text,uuid) from public, anon;
grant execute on function public.can_manage_daily_site_record(uuid, uuid, uuid) to authenticated;
grant execute on function public.daily_site_authorised_sites() to authenticated;
grant execute on function public.create_daily_site_entry_draft_for_site(uuid,uuid,date,text,text,text,integer,text,numeric,numeric,text,numeric,numeric,text,text,uuid) to authenticated;
