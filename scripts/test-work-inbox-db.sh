#!/usr/bin/env bash
# BD-INBOX-01 (Stage 3) — Work Inbox read-state database test runner.
# Spins up a disposable PostgreSQL 17 cluster, applies every migration in order,
# then runs the Work Inbox read-state test suite. No hosted Supabase is touched.
set -euo pipefail
...
test_root="$(mktemp -d /tmp/bd-inbox-db.XXXXXX)"
data_dir="$test_root/data"
socket_dir="$test_root/socket"
log_file="$test_root/postgres.log"
port="${BD_INBOX_TEST_PORT:-55448}"
...
"${psql_cmd[@]}" -f "$repo_dir/supabase/tests/work_inbox_read_state_test.sql"
