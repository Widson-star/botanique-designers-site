...
test_root="$(mktemp -d /tmp/bd-fin-fund-requests-db.XXXXXX)"
data_dir="$test_root/data"
socket_dir="$test_root/socket"
log_file="$test_root/postgres.log"
port="${BD_FIN_FUND_REQUESTS_TEST_PORT:-55442}"
...
"${psql_cmd[@]}" -f "$repo_dir/supabase/tests/claim_backed_fund_requests_test.sql"
