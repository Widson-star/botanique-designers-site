#!/usr/bin/env bash
# BD-PEOPLE-01 (Stage 5) — People and project engagements database test runner.
# Spins up a disposable PostgreSQL 17 cluster, applies every migration in order,
# then runs the People and engagements test suite. No hosted Supabase is touched.
set -euo pipefail
...
test_root="$(mktemp -d /tmp/bd-people-db.XXXXXX)"
data_dir="$test_root/data"
socket_dir="$test_root/socket"
log_file="$test_root/postgres.log"
port="${BD_PEOPLE_TEST_PORT:-55449}"
