#!/usr/bin/env bash
# BD-REPORTS-01A — Daily Site period-compliance range source database test runner.
# Spins up a disposable PostgreSQL 17 cluster, applies every migration in order,
# then runs the Reports range-source test suite. No hosted Supabase is touched.
set -euo pipefail
...
test_root="$(mktemp -d /tmp/bd-reports-db.XXXXXX)"
data_dir="$test_root/data"
socket_dir="$test_root/socket"
log_file="$test_root/postgres.log"
port="${BD_REPORTS_TEST_PORT:-55446}"
...
"${psql_cmd[@]}" -f "$repo_dir/supabase/tests/reports_daily_site_range_compliance_test.sql"
