...
// Database remains the source of truth; the UI mirrors the authority boundary.
import { ROLES } from "../constants/roles";
import { PROJECT_STAGES, PROJECT_STATUSES } from "../constants/projectStatus";

// Completed is terminal delivery. Archive is a separate record flag, not a stage.
...
export function canMarkCompleted(role, project) {
  return isOwner(role) && !project.archived && !["Completed", "Cancelled"].includes(project.status);
}
export function canCancel(role, project) {
  return isOwner(role) && !project.archived && !["Cancelled", "Completed"].includes(project.status);
}
export function canClassifyDesignOnly(role, project) {
  return isOwner(role) && !project.archived && !["Design-only", "Completed", "Cancelled"].includes(project.status);
}
...
export function hasOwnerMaterialActions(role, project) {
  return canActivate(role, project) || canMarkCompleted(role, project) || canCancel(role, project) || canClassifyDesignOnly(role, project) || canArchive(role, project) || canRestore(role, project);
}
