...
export default function AdminFinance() {
  ...
  const unknownCompensation = approvedCompensation.filter((item) => paymentPositionForCompensation(item.id)?.paymentStatus === "payment_history_unknown");
  const knownCompensation = approvedCompensation.filter((item) => paymentPositionForCompensation(item.id)?.paymentStatus !== "payment_history_unknown");
  const approvedCompensationAmount = approvedCompensation.reduce((sum, item) => sum + Number(item.approvedAmount || 0), 0);
  const compensationOutstanding = knownCompensation.reduce((sum, item) => sum + Number(paymentPositionForCompensation(item.id)?.balanceAmount || 0), 0);
  const compensationPaid = knownCompensation.reduce((sum, item) => sum + Number(paymentPositionForCompensation(item.id)?.paidAmount || 0), 0);
  ...
  const paymentPositionDisplay = unknownCompensation.length ? "—" : money(compensationPaid);
  const outstandingDisplay = unknownCompensation.length ? "—" : money(compensationOutstanding);
  ...
  return <section className="space-y-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5" aria-label="Finance areas">{AREAS.map((area) => {
      ...
      const summary = areaSummary(area.id, { awaitingCosts, awaitingCostAmount, advancesIssued, advancesIssuedAmount, compensationOutstanding, approvedCompensation, approvedCompensationAmount, unknownCompensation });
...
function areaSummary(id, data) {
  ...
  if (id === "staff-compensation") {
    if (data.unknownCompensation.length) return { label: "Approved", value: money(data.approvedCompensationAmount), hint: `${data.unknownCompensation.length} payment ${data.unknownCompensation.length === 1 ? "history" : "histories"} to confirm` };
    return { label: "Outstanding", value: money(data.compensationOutstanding), hint: `${data.approvedCompensation.length} approved ${data.approvedCompensation.length === 1 ? "record" : "records"}` };
  }
