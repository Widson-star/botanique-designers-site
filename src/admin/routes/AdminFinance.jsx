...
export default function AdminFinance() {
  ...
  const activity = [
    ...
    ...compensations.map((item) => ({ id: `pay-${item.id}`, title: personName[item.personId] || "Staff member", detail: `Staff pay · ${plainStatus(item.lifecycle)}`, amount: Number(item.approvedAmount ?? item.submittedAmount ?? 0), at: item.updatedAt || item.decidedAt || item.submittedAt || item.createdAt || "", to: `/admin/finance/staff-compensation/${item.id}` })),
...
function areaSummary(id, data) {
  ...
  if (id === "staff-compensation") {
    if (data.unknownPay.length) return { label: "Approved", value: money(data.approvedPayAmount), hint: `${data.unknownPay.length} payment ${data.unknownPay.length === 1 ? "history" : "histories"} to confirm` };
    return { label: "Outstanding", value: money(data.payOutstanding), hint: `${data.approvedPay.length} approved ${data.approvedPay.length === 1 ? "record" : "records"}` };
  }
