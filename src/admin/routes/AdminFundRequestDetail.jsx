...
import {
  ...
  canWithdrawFundRequest, FUND_REQUEST_STATUSES,
...
const EVENT_LABELS = {
  ...
  draft_updated: "Advance updated",
  submitted: "Submitted for Principal decision",
  amendment_requested: "Correction requested",
  resubmitted: "Resubmitted",
  approved: "Approved",
  rejected: "Rejected",
  withdrawn: "Withdrawn",
  cancelled: "Cancelled",
  principal_direct_authorised: "Principal authorised advance directly",
};
...
export default function AdminFundRequestDetail() {
  ...
  if (status === "loading" && !request) return <p className="text-sm text-gray-600">Loading advance…</p>;
  if (!request) return <section>
    <h1 className="text-2xl font-semibold">Advance unavailable</h1>
    <p className="mt-2 text-sm text-gray-600">It may not exist or you may not have project authority.</p>
    <Link to="/admin/fund-requests" className="mt-4 inline-block text-sm font-medium text-botanique-green">Back to Advances</Link>
  </section>;

  const isAdvance = request.intendedCustodyType === ADVANCE_CUSTODY;
  ...
  async function act(operation) {
    ...
    if (!result.ok) {
      setError(result.stale
        ? "This advance changed elsewhere. The latest version has been reloaded."
        : result.error);
    }
    await refresh();
  ...
  if (!isAdvance) {
    return (
      <section className="mx-auto max-w-4xl space-y-4">
        <Link to="/admin/fund-requests" className="text-sm font-medium text-botanique-green hover:underline">← Advances</Link>
        <div className="rounded-xl border border-stone-200 bg-white p-5">
          <h1 className="text-xl font-semibold">Legacy direct-payment authority</h1>
          <p className="mt-2 text-sm text-gray-600">
            This record predates the settled Finance architecture and is not an Advance. It is preserved for audit history and will not be relabelled as one.
          </p>
          <p className="mt-3 text-sm"><span className="font-medium">Project:</span> {project?.projectName || "Project"}</p>
          <p className="mt-1 text-sm"><span className="font-medium">Amount:</span> {money(request.totalRequestedAmount)}</p>
          <p className="mt-1 text-sm"><span className="font-medium">Status:</span> {FUND_REQUEST_STATUSES[request.status]}</p>
        </div>
      </section>
    );
  }
  ...
  return <section className="mx-auto max-w-6xl">
    ...
    <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-600">
          {project?.projectName || "Project"} · Accountable advance · round {request.submissionRound}
        </p>
      </div>
    ...
    <div className="mt-5 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
      {request.status !== "approved"
        ? "This request asks for an Advance. No money has moved merely because the request exists."
        : position.releaseState === "none"
          ? "This Advance is approved but has not been issued yet."
          : `${money(position.releasedAmount)} of the approved ${money(position.authorisedAmount)} has been issued.`}
    </div>
    ...
    {request.authorityType === "principal_direct" && <div className="mt-4 rounded-lg border border-sky-200 bg-sky-50 p-4 text-sm text-sky-950">
      <p className="font-semibold">Principal direct advance</p>
      <p className="mt-1">Authorised directly by {actorName(request.directAuthorityActorId)} rather than submitted for a separate approval.</p>
    </div>}
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[2fr_1fr]">
      <div className="space-y-5">
        <div className="rounded-lg border border-stone-200 bg-white p-5">
          <h2 className="font-semibold">Advance summary</h2>
          <dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2">
            <div><dt className="text-gray-500">Purpose</dt><dd className="mt-1">{request.purpose}</dd></div>
            {request.custodianProfileId && <div><dt className="text-gray-500">Accountable person</dt><dd className="mt-1">{actorName(request.custodianProfileId)}</dd></div>}
            {request.requesterId && <div><dt className="text-gray-500">Requested by</dt><dd className="mt-1">{actorName(request.requesterId)}</dd></div>}
            <div>
              <dt className="text-gray-500">{request.status === "approved" ? "Approved advance" : "Requested advance"}</dt>
        ...
        <div className="overflow-hidden rounded-lg border border-stone-200 bg-white">
          <div className="px-5 py-4"><h2 className="font-semibold">Approved Project Costs</h2></div>
          <div className="divide-y divide-stone-100">{allocations.map((allocation) => <div key={allocation.id} className="grid gap-1 px-5 py-4 text-sm sm:grid-cols-[1fr_auto] sm:items-center">
            <div>
              ...
              <p className="text-xs text-gray-500">Approved cost {money(allocation.claimApprovedTotal)}</p>
            </div>
        ...
        {request.status === "approved" && <FundReleaseSection
          ...
          onReverse={(releaseId, version, nextReason) => act(() => reverseRelease(releaseId, version, nextReason))}
          ...
          onDecideAcquittal={(acquittalId, version, decision, nextReason) => act(() => decideAcquittal(acquittalId, version, decision, nextReason))}
        />}
        ...
        <div className="rounded-lg border border-stone-200 bg-white p-5">
          ...
          <ol className="mt-4 space-y-4">{events.map((event) => <li key={event.id} className="border-l-2 border-botanique-green pl-4 text-sm">
            <p className="font-semibold">{EVENT_LABELS[event.eventType] || plain(event.eventType)}</p>
      ...
      <aside className="space-y-4">
        {(canDecideFundRequest(request, role) || canCancelFundRequest(request, role)) && <div className="rounded-lg border border-stone-200 bg-white p-5">
          ...
          {canDecideFundRequest(request, role) && <p className="mt-2 text-xs text-gray-600">
            Approving allows Botanique to issue up to {money(request.totalRequestedAmount)} to the accountable person. It does not itself move money.
          </p>}
          ...
          {canDecideFundRequest(request, role) && <div className="mt-3 grid gap-2">
            <button disabled={working} onClick={() => act(() => decideRequest(request.id, request.version, "approved", reason))} className="min-h-11 rounded-md bg-botanique-green px-3 text-sm font-semibold text-white">Approve advance</button>
            <button disabled={working || !reason.trim()} onClick={() => act(() => decideRequest(request.id, request.version, "amendment_requested", reason))} className="min-h-11 rounded-md border border-amber-400 px-3 text-sm font-semibold text-amber-900 disabled:opacity-50">Request correction</button>
            <button disabled={working || !reason.trim()} onClick={() => act(() => decideRequest(request.id, request.version, "rejected", reason))} className="min-h-11 rounded-md border border-red-300 px-3 text-sm font-semibold text-red-700 disabled:opacity-50">Reject advance</button>
          </div>}
          {canCancelFundRequest(request, role) && position.releaseState === "none" && <button disabled={working || !reason.trim()} onClick={() => act(() => cancelRequest(request.id, request.version, reason))} className="mt-3 min-h-11 w-full rounded-md border border-red-300 px-3 text-sm font-semibold text-red-700 disabled:opacity-50">Cancel approved advance</button>}
        </div>}
        ...
        {(canEditFundRequest(request, role, currentUserId) || canWithdrawFundRequest(request, role, currentUserId)) && <div className="rounded-lg border border-stone-200 bg-white p-5">
          ...
          <div className="mt-3 grid gap-2">
            {canEditFundRequest(request, role, currentUserId) && <button onClick={() => navigate(`/admin/fund-requests/${request.id}/edit`)} className="min-h-11 rounded-md border border-stone-300 px-3 text-sm font-semibold">Edit advance</button>}
