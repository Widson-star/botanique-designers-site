...
// A Project Cost is identified by what it was for. The recipient/crew string is
// often raw site arithmetic ("(Mason 1200 and 2 casuals @500} Ksh 2200…") and is
// not an identity; it stays available in the summary below.
  ...
  return <section className="mx-auto max-w-6xl">
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-[2fr_1fr]">
      <div className="space-y-5">
        <div className="rounded-lg border border-stone-200 bg-white p-5"><h2 className="font-semibold">Project Cost summary</h2><dl className="mt-4 grid gap-4 text-sm sm:grid-cols-2"><div><dt className="text-gray-500">Purpose</dt><dd className="mt-1">{claim.purpose}</dd></div><div><dt className="text-gray-500">Service date</dt><dd className="mt-1">{claim.serviceDate}</dd></div><div><dt className="text-gray-500">Recipient type</dt><dd className="mt-1 capitalize">{claim.recipientType.replaceAll("_", " ")}</dd></div><div><dt className="text-gray-500">Requester</dt><dd className="mt-1">{actorName(claim.requesterId)}</dd></div>{claim.deciderId && <div><dt className="text-gray-500">Principal decision by</dt><dd className="mt-1">{actorName(claim.deciderId)}</dd></div>}<div><dt className="text-gray-500">Authoritative amount</dt><dd className="mt-1 text-xl font-semibold">{money(claim.approvedTotal ?? claim.submittedTotal ?? lines.reduce((sum, line) => sum + line.lineTotal, 0))}</dd></div></dl></div>
