        ...
        <div className="min-w-0 overflow-visible rounded-xl border border-stone-200 bg-white">
          ...
          {visible.length > 0 && <>
            <div className="hidden overflow-x-auto md:block">
              <table className="w-full text-left text-[12.5px]">
                ...
                <tbody className="divide-y divide-stone-100">{visible.map((item,index)=>{
                  ...
                  return <tr key={item.id} className="hover:bg-[#fbfbfa]"><td className="px-3.5 py-3 text-gray-500">{index+1}</td><td className="whitespace-nowrap px-3.5 py-3 text-gray-600">{date(item.serviceDate)}</td><td className="px-3.5 py-3 font-medium text-botanique-charcoal">{person?.fullName || "Person"}</td><td className="px-3.5 py-3 text-gray-600">{typeLabels[item.compensationType] || "Other"}</td><td className="max-w-[180px] px-3.5 py-3 text-gray-600"><span className="line-clamp-2">{item.projectId ? projectMap.get(item.projectId)?.projectName || "Project" : "—"}</span></td><td className="px-3.5 py-3"><Chip tone={tones[item.lifecycle]}>{labels[item.lifecycle]}</Chip></td><td className="whitespace-nowrap px-3.5 py-3 text-right font-semibold tabular-nums">{money(item.lifecycle === "approved" ? item.approvedAmount : item.submittedAmount)}</td><td className="whitespace-nowrap px-3.5 py-3 text-right tabular-nums text-gray-600">{unknown || position?.paidAmount == null ? "—" : money(position.paidAmount)}</td><td className="whitespace-nowrap px-3.5 py-3 text-right font-semibold tabular-nums">{unknown || position?.balanceAmount == null ? "—" : money(position.balanceAmount)}</td><td className="px-3.5 py-3 text-right"><PayActionMenu item={item} position={position} role={role} currentUserId={currentUserId} open={openMenu===item.id} onToggle={()=>setOpenMenu(openMenu===item.id?null:item.id)}/></td></tr>;
                })}</tbody>
              </table>
            </div>
            ...
            <ul className="space-y-2.5 p-3 md:hidden">{visible.map((item,index)=>{
              ...
              return <li key={item.id} className="rounded-xl border border-stone-200 bg-white p-3.5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><Link to={`/admin/finance/staff-compensation/${item.id}`} className="font-semibold text-botanique-green">{person?.fullName || "Person"}</Link><p className="mt-1 text-[11.5px] text-gray-500">#{index+1} · {date(item.serviceDate)} · {typeLabels[item.compensationType]}</p></div><Chip tone={tones[item.lifecycle]}>{labels[item.lifecycle]}</Chip></div><p className="mt-2 line-clamp-2 text-[12px] text-gray-600">{item.projectId ? projectMap.get(item.projectId)?.projectName || "Project" : "No Project context"}</p><dl className="mt-2.5 grid grid-cols-3 gap-2 border-t border-stone-100 pt-2.5 text-[11.5px]"><div><dt className="text-gray-500">Total</dt><dd className="font-semibold tabular-nums">{money(item.lifecycle === "approved" ? item.approvedAmount : item.submittedAmount)}</dd></div><div><dt className="text-gray-500">Paid</dt><dd>{unknown || position?.paidAmount == null ? "—" : money(position.paidAmount)}</dd></div><div><dt className="text-gray-500">Balance</dt><dd>{unknown || position?.balanceAmount == null ? "—" : money(position.balanceAmount)}</dd></div></dl><div className="mt-2.5 flex justify-end"><PayActionMenu item={item} position={position} role={role} currentUserId={currentUserId} open={openMenu===item.id} onToggle={()=>setOpenMenu(openMenu===item.id?null:item.id)}/></div></li>;
            })}</ul>
...
function PayActionMenu({ item, position, role, currentUserId, open, onToggle }) {
