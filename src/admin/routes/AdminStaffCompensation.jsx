...
export default function AdminStaffCompensation() {
  ...
  const historyUnknown = approved.filter(
    (item) => paymentPositionForCompensation(item.id)?.paymentStatus === "payment_history_unknown"
  );
  const knownApproved = approved.filter(
    (item) => paymentPositionForCompensation(item.id)?.paymentStatus !== "payment_history_unknown"
  );
  const totalApproved = approved.reduce((sum, item) => sum + Number(item.approvedAmount || 0), 0);
  const totalPaid = knownApproved.reduce(
    (sum, item) => sum + Number(paymentPositionForCompensation(item.id)?.paidAmount || 0), 0
  );
  const outstanding = knownApproved.reduce(
    (sum, item) => sum + Number(paymentPositionForCompensation(item.id)?.balanceAmount || 0), 0
  );
  const awaiting = compensations.filter((item) => item.lifecycle === "awaiting_review");
  const partPaid = approved.filter(
    (item) => paymentPositionForCompensation(item.id)?.paymentStatus === "part_paid"
  );
  ...
  const recordedPayments = payments
    ...
    .map((payment) => {
      ...
      return {
        payment,
        compensation,
        person: personMap.get(compensation?.personId),
        project: projectMap.get(compensation?.projectId),
      };
    });
    ...
    <section className="space-y-4">
      <header className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <p className="mt-0.5 max-w-3xl text-[13px] text-gray-600">
            Compensation belongs to a person, Project context is optional, approval and payment remain separate.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2.5">
          <Link
            to="/admin/finance/staff-compensation/new"
            className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-botanique-green px-5 text-[13px] font-semibold text-white hover:bg-botanique-dark"
          >
            <span className="text-lg leading-none">+</span> New compensation
          </Link>
          <Link
            to="/admin/approvals"
            className="inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-[12.5px] font-semibold text-botanique-charcoal hover:bg-stone-50"
          >
            <Glyph name="check" /> Open Approvals <span aria-hidden="true">→</span>
          </Link>
        </div>
      </header>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
        <div className="min-w-0 overflow-hidden rounded-xl border border-stone-200 bg-white">
          <div className="grid gap-3 border-b border-stone-100 p-4 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_auto]">
            <Filter label="Person" value={personId} onChange={setPersonId}>
              ...
              {people.slice().sort((a, b) => a.fullName.localeCompare(b.fullName)).map((person) => (
            ...
            <Filter label="Project (optional)" value={projectId} onChange={setProjectId}>
              ...
              {projects.slice().sort((a, b) => a.projectName.localeCompare(b.projectName)).map((project) => (
          ...
          {status !== "loading" && !visible.length && (
            <div className="m-4 flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-4">
              <Disc name="people" tone="unbuilt" size="h-9 w-9" />
              <p className="text-[12.5px] text-gray-600">{compensations.length ? "No compensation matches these filters." : "No Staff Compensation has been recorded yet."}</p>
            </div>
          )}
          ...
          {visible.length > 0 && <>
            <div className="hidden overflow-x-auto md:block">
              <table className="w-full text-left text-[12.5px]">
                <thead className="border-b border-stone-100 bg-[#fbfbfa] text-[10.5px] uppercase tracking-wide text-gray-500">
                  <tr>
                    {['#', 'Date', 'Person', 'Type', 'Project', 'Status'].map((heading) => <th key={heading} className="px-3.5 py-3 font-medium">{heading}</th>)}
                    {['Total', 'Paid', 'Balance', 'Action'].map((heading) => <th key={heading} className="px-3.5 py-3 text-right font-medium">{heading}</th>)}
                  </tr>
                </thead>
                <tbody className="divide-y divide-stone-100">
                  {visible.map((item, index) => {
                    ...
                    const action = item.lifecycle === "awaiting_review" && role === "owner"
                      ? "Review"
                      : item.lifecycle === "draft" && item.requesterId === currentUserId && role === "manager"
                        ? "Edit"
                        : "View";
                    const to = action === "Review"
                      ? `/admin/approvals/staff-compensation:${item.id}`
                      : action === "Edit"
                        ? `/admin/finance/staff-compensation/${item.id}/edit`
                        : `/admin/finance/staff-compensation/${item.id}`;
                    return (
                      <tr key={item.id} className="hover:bg-[#fbfbfa]">
                        <td className="px-3.5 py-3 text-gray-500">{index + 1}</td>
                        <td className="whitespace-nowrap px-3.5 py-3 text-gray-600">{date(item.serviceDate)}</td>
                        <td className="px-3.5 py-3"><div className="flex items-center gap-2"><Initials name={person?.fullName} /><span className="font-medium text-botanique-charcoal">{person?.fullName || "Person"}</span></div></td>
                        <td className="px-3.5 py-3 text-gray-600">{typeLabels[item.compensationType] || "Other"}</td>
                        <td className="max-w-[180px] px-3.5 py-3 text-gray-600"><span className="line-clamp-2">{item.projectId ? projectMap.get(item.projectId)?.projectName || "Project" : "—"}</span></td>
                        <td className="px-3.5 py-3"><Chip tone={tones[item.lifecycle]}>{labels[item.lifecycle]}</Chip></td>
                        <td className="whitespace-nowrap px-3.5 py-3 text-right font-semibold tabular-nums">{money(item.lifecycle === "approved" ? item.approvedAmount : item.submittedAmount)}</td>
                        <td className="whitespace-nowrap px-3.5 py-3 text-right tabular-nums text-gray-600">{unknown || position?.paidAmount == null ? "—" : money(position.paidAmount)}</td>
                        <td className="whitespace-nowrap px-3.5 py-3 text-right font-semibold tabular-nums">{unknown || position?.balanceAmount == null ? "—" : money(position.balanceAmount)}</td>
                        <td className="px-3.5 py-3 text-right"><Link to={to} className="font-semibold text-botanique-green hover:underline">{action}</Link></td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <ul className="space-y-2.5 p-3 md:hidden">
              {visible.map((item, index) => {
                ...
                const unknown = position?.paymentStatus === "payment_history_unknown";
                return (
                  <li key={item.id} className="rounded-xl border border-stone-200 bg-white p-3.5">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0"><div className="flex items-center gap-2"><Initials name={person?.fullName} /><Link to={`/admin/finance/staff-compensation/${item.id}`} className="font-semibold text-botanique-green">{person?.fullName || "Person"}</Link></div><p className="mt-1 text-[11.5px] text-gray-500">#{index + 1} · {date(item.serviceDate)} · {typeLabels[item.compensationType]}</p></div>
                      <Chip tone={tones[item.lifecycle]}>{labels[item.lifecycle]}</Chip>
                    </div>
                    <p className="mt-2 line-clamp-2 text-[12px] text-gray-600">{item.projectId ? projectMap.get(item.projectId)?.projectName || "Project" : "No Project context"}</p>
                    <dl className="mt-2.5 grid grid-cols-3 gap-2 border-t border-stone-100 pt-2.5 text-[11.5px]">
                      <div><dt className="text-gray-500">Total</dt><dd className="font-semibold tabular-nums">{money(item.lifecycle === "approved" ? item.approvedAmount : item.submittedAmount)}</dd></div>
                      <div><dt className="text-gray-500">Paid</dt><dd>{unknown || position?.paidAmount == null ? "—" : money(position.paidAmount)}</dd></div>
                      <div><dt className="text-gray-500">Balance</dt><dd>{unknown || position?.balanceAmount == null ? "—" : money(position.balanceAmount)}</dd></div>
                    </dl>
                  </li>
                );
              })}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-stone-100 px-4 py-3 text-[11px] text-gray-500">
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-xl border border-stone-200 bg-white p-4">
            <div className="flex items-center gap-2.5"><Disc name="bank" tone="brand" size="h-8 w-8" /><h2 className="text-[13.5px] font-semibold">Recent payment activity</h2></div>
            {recordedPayments.length ? (
              <ul className="mt-3 divide-y divide-stone-100">
                {recordedPayments.map(({ payment, person, project }) => (
                  <li key={payment.id} className="py-3 first:pt-0"><div className="flex gap-2.5"><Initials name={person?.fullName} /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className="truncate text-[12px] font-semibold">{person?.fullName || "Person"}</p><span className="shrink-0 text-[11px] font-semibold text-emerald-700">+{money(payment.amount)}</span></div><p className="mt-0.5 truncate text-[10.5px] text-gray-500">{project?.projectName || "No Project context"}</p><div className="mt-0.5 flex justify-between gap-2 text-[10px] text-gray-400"><span>{methodLabels[payment.paymentChannel] || "Payment"}</span><span>{date(payment.paidAt)}</span></div></div></div></li>
                ))}
              </ul>
            ) : <p className="mt-3 text-[11.5px] text-gray-500">No Staff Compensation payment has been recorded yet.</p>}
...
function Metric({ icon, tone, label, value, hint }) {
  return <div className="flex min-h-[112px] items-center gap-3 rounded-xl border border-stone-200 bg-white p-4"><Disc name={icon} tone={tone} size="h-10 w-10" /><div><p className="text-[11.5px] text-gray-500">{label}</p><p className="mt-1 text-[20px] font-semibold tabular-nums text-botanique-charcoal">{value}</p><p className="mt-1 text-[10.5px] text-gray-500">{hint}</p></div></div>;
}
function Filter({ label, value, onChange, children }) {
  return <label className="text-[11.5px] font-medium text-gray-600">{label}<select value={value} onChange={(event) => onChange(event.target.value)} className="mt-1 block min-h-10 w-full rounded-lg border border-stone-300 bg-white px-3 text-[12.5px]">{children}</select></label>;
}
function Attention({ icon, tone, label, hint, count, to }) {
  return <Link to={to} className="flex min-h-[68px] items-center gap-2.5 py-2.5"><Disc name={icon} tone={tone} size="h-8 w-8" /><span className="min-w-0 flex-1"><span className="block text-[11.5px] font-semibold text-botanique-charcoal">{label}</span><span className="mt-0.5 block text-[10px] text-gray-500">{hint}</span></span><strong className="text-[12px] tabular-nums">{count}</strong><span className="text-gray-400">›</span></Link>;
}
function Initials({ name }) {
  const initials = String(name || "P").split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join("").toUpperCase();
  return <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-botanique-green text-[9px] font-bold text-white">{initials}</span>;
}
