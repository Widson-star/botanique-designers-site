...
const eventLabels = {
  created: "Draft created", amended: "Record amended", submitted: "Submitted for review",
  ...
  payment_history_confirmed: "Historical payment position confirmed",
  legacy_imported: "Imported from Project Costs",
};
...
export default function AdminStaffCompensationDetail() {
  ...
  const {
    compensations, eventsByCompensation, loadEvents, submitRecord, withdrawRecord, cancelRecord,
    paymentsForCompensation, paymentPositionForCompensation, recordPayment, reversePayment,
    confirmPaymentHistory, refresh, status,
  ...
  if (status === "loading" && !record) return <p className="text-sm text-gray-600">Loading Staff Compensation…</p>;
  if (!record) return <section><h1 className="text-xl font-semibold">Staff Compensation unavailable</h1><Link to="/admin/finance/staff-compensation" className="mt-3 inline-block text-sm font-semibold text-botanique-green">Back to Staff Compensation</Link></section>;
  ...
  return <section className="mx-auto max-w-6xl space-y-4">
    <Link to="/admin/finance/staff-compensation" className="text-sm font-semibold text-botanique-green hover:underline">← Staff Compensation</Link>
    <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-botanique-green">Finance · Staff Compensation</p><h1 className="mt-1 text-2xl font-semibold">{person?.fullName || "Person"}</h1><p className="mt-1 text-[13px] text-gray-600">{shortDate(record.serviceDate)} · {typeLabel(record.compensationType)}{project ? ` · ${project.projectName}` : " · No Project context"}</p></div><Chip tone={tones[record.lifecycle]}>{labels[record.lifecycle]}</Chip></header>
    {error && <p className="rounded-lg border border-red-200 bg-red-50 p-3 text-[13px] text-red-800">{error}</p>}

    {unknownHistory && <section className="rounded-xl border border-amber-200 bg-amber-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="max-w-3xl"><h2 className="text-[13.5px] font-semibold text-amber-950">Historical payment position not yet confirmed</h2><p className="mt-1 text-[12px] leading-relaxed text-amber-900">This compensation was migrated from the older Project Cost record. The Hub knows the approved obligation, but it does not yet know how much had already been paid. Paid and Balance therefore remain blank instead of assuming the record is unpaid.</p></div>{role === "owner" && !historyOpen && <button type="button" onClick={()=>setHistoryOpen(true)} className="min-h-9 rounded-lg border border-amber-300 bg-white px-3 text-[11.5px] font-semibold text-amber-900">Resolve payment history</button>}</div>{historyOpen && <div className="mt-3 border-t border-amber-200 pt-3"><label className="block text-[12px] font-medium text-amber-950">Confirmation note<textarea rows={2} value={historyReason} onChange={(e)=>setHistoryReason(e.target.value)} className="mt-1 w-full rounded-lg border border-amber-300 bg-white px-3 py-2 text-botanique-charcoal" placeholder="Record what was checked before marking the historical payment position as known."/></label><p className="mt-1.5 text-[11px] text-amber-900">Only confirm after checking the old payment evidence. This action does not create a payment transaction.</p><div className="mt-2 flex gap-2"><button disabled={working || !historyReason.trim()} onClick={confirmHistory} className="min-h-9 rounded-lg bg-botanique-green px-3 text-[11.5px] font-semibold text-white disabled:opacity-50">Confirm history reviewed</button><button onClick={()=>{setHistoryOpen(false);setHistoryReason("");}} className="min-h-9 rounded-lg border border-amber-300 bg-white px-3 text-[11.5px] font-semibold">Not yet</button></div></div>}</section>}

    <section className="rounded-xl border border-stone-200 bg-white p-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-[14px] font-semibold">Position</h2><p className="mt-1 text-[12px] text-gray-500">Approval confirms the obligation. Payment is recorded separately here.</p></div>{position?.paymentStatus && record.lifecycle === "approved" && <Chip tone={position.paymentStatus === "paid" ? "settled" : position.paymentStatus === "part_paid" ? "waiting" : "neutral"}>{paymentLabels[position.paymentStatus] || position.paymentStatus}</Chip>}</div><dl className="mt-4 grid grid-cols-3 gap-3 border-t border-stone-100 pt-4"><Fact label={record.lifecycle === "approved" ? "Approved" : "Submitted"} value={money(record.lifecycle === "approved" ? record.approvedAmount : record.submittedAmount)}/><Fact label="Paid" value={unknownHistory || position?.paidAmount == null ? "—" : money(position.paidAmount)}/><Fact label="Balance" value={unknownHistory || position?.balanceAmount == null ? "—" : money(position.balanceAmount)} attention={!unknownHistory && Number(position?.balanceAmount || 0) > 0}/></dl></section>

    <div className="grid gap-4 lg:grid-cols-[1.15fr_.85fr]">
      <section className="rounded-xl border border-stone-200 bg-white p-4"><h2 className="text-[14px] font-semibold">Compensation record</h2><dl className="mt-3 divide-y divide-stone-100 text-[12.5px]"><Row label="Person" value={person?.fullName || "Person"}/><Row label="Type" value={typeLabel(record.compensationType)}/><Row label="Date" value={shortDate(record.serviceDate)}/><Row label="Project" value={project?.projectName || "None"}/><Row label={record.directAuthorityActorId ? "Authorised by" : "Requester"} value={profileName(profileMap.get(record.directAuthorityActorId || record.requesterId))}/><Row label="Round" value={String(record.requestRound)}/>{record.legacySourceClaimId && <Row label="Legacy source" value={<Link to={`/admin/site-costs/${record.legacySourceClaimId}`} className="font-semibold text-botanique-green hover:underline">View original Project Cost</Link>}/>}</dl><div className="mt-4 border-t border-stone-100 pt-3"><p className="text-[11.5px] font-medium text-gray-500">Purpose</p><p className="mt-1 whitespace-pre-wrap text-[13px] leading-relaxed text-botanique-charcoal">{record.description}</p></div>
...
function Fact({label,value,attention}){return <div><dt className="text-[11.5px] text-gray-500">{label}</dt><dd className={`mt-1 text-[18px] font-semibold tabular-nums ${attention ? "text-amber-800" : "text-botanique-charcoal"}`}>{value}</dd></div>}
...
function typeLabel(value){return ({compensation:"Compensation",allowance:"Allowance",bonus:"Bonus",other:"Other"})[value] || value}
