...
const eventLabels = { created: "Draft created", amended: "Record amended", submitted: "Submitted for review", amendment_requested: "Amendment requested", resubmitted: "Resubmitted", approved: "Approved", rejected: "Rejected", withdrawn: "Withdrawn", cancelled: "Cancelled" };
const paymentLabels = { unpaid: "Unpaid", part_paid: "Part-paid", paid: "Paid" };
...
export default function AdminStaffCompensationDetail() {
  ...
  const { compensations, eventsByCompensation, loadEvents, submitRecord, withdrawRecord, cancelRecord, paymentsForCompensation, paymentPositionForCompensation, recordPayment, reversePayment, refresh, status } = useStaffCompensation();
  ...
  const payable = role === "owner" && record.lifecycle === "approved" && Number(position?.balanceAmount || 0) > 0;
  const cancellable = role === "owner" && record.lifecycle === "approved" && Number(position?.paidAmount || 0) === 0;
