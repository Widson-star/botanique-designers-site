...
export default function AdminStaffCompensationDetail() {
  ...
  const {
    compensations, eventsByCompensation, loadEvents, submitRecord, withdrawRecord,
    cancelRecord, paymentsForCompensation, paymentPositionForCompensation,
    recordPayment, reversePayment, confirmPaymentHistory, correctPaymentHistory,
    refresh, status,
  ...
  const [correctingHistory, setCorrectingHistory] = useState(false);
  const [historyCorrectionReason, setHistoryCorrectionReason] = useState("");
  ...
  const historicalAmountValid = historicalPaidAmount !== ""
    && Number(historicalPaidAmount) >= 0
    && Number(historicalPaidAmount) <= Number(record.approvedAmount || 0);
  ...
  async function confirmHistory() {
    if (!historicalAmountValid || !historyReason.trim()) return;
    const ok = await act(() => confirmPaymentHistory(record.id, record.version, Number(historicalPaidAmount), historyReason.trim()));
    if (ok) {
      setHistoryReason("");
      setHistoricalPaidAmount("");
      setHistoryOpen(false);
    }
  }
  async function correctHistory() {
    if (!historyCorrectionReason.trim()) return;
    const ok = await act(() => correctPaymentHistory(record.id, record.version, historyCorrectionReason.trim()));
    if (ok) {
      setCorrectingHistory(false);
      setHistoryCorrectionReason("");
    }
  }
  ...
  return <section className="mx-auto max-w-6xl space-y-4">
    ...
    {unknownHistory && <section id="payment-history" className="rounded-xl border border-sky-200 bg-sky-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div className="max-w-3xl"><h2 className="text-[13.5px] font-semibold text-sky-950">Historical payment position not yet confirmed</h2><p className="mt-1 text-[12px] leading-relaxed text-sky-900">This Staff Pay record came from the older Project Costs workflow. Confirm how much of the approved amount had already been paid before the record moved to Staff Pay. The Hub will not invent an old payment date, method or reference.</p></div>{role === "owner" && !historyOpen && <button type="button" onClick={()=>setHistoryOpen(true)} className="min-h-9 rounded-lg border border-sky-300 bg-white px-3 text-[11.5px] font-semibold text-sky-900">Resolve payment history</button>}</div>{historyOpen && <div className="mt-3 grid gap-3 border-t border-sky-200 pt-3 sm:grid-cols-2"><label className="block text-[12px] font-medium text-sky-950">Amount already paid (KES)<input type="number" min="0" max={record.approvedAmount || undefined} step="0.01" value={historicalPaidAmount} onChange={(e)=>setHistoricalPaidAmount(e.target.value)} className="mt-1 min-h-10 w-full rounded-lg border border-sky-300 bg-white px-3 py-2 text-botanique-charcoal" placeholder="0.00"/></label><div className="text-[11px] leading-relaxed text-sky-900 sm:pt-6">Enter <strong>0</strong> only if the evidence confirms nothing had been paid. Enter the actual historical amount if some or all of it had already been settled.</div><label className="block text-[12px] font-medium text-sky-950 sm:col-span-2">Confirmation note<textarea rows={2} value={historyReason} onChange={(e)=>setHistoryReason(e.target.value)} className="mt-1 w-full rounded-lg border border-sky-300 bg-white px-3 py-2 text-botanique-charcoal" placeholder="Record what evidence or records were checked."/></label><p className="text-[11px] text-sky-900 sm:col-span-2">This establishes the opening paid position. It does not create a payment transaction. Any remaining balance is settled below using the real payment date and method.</p><div className="flex gap-2 sm:col-span-2"><button disabled={working || !historicalAmountValid || !historyReason.trim()} onClick={confirmHistory} className="min-h-9 rounded-lg bg-botanique-green px-3 text-[11.5px] font-semibold text-white disabled:opacity-50">Confirm payment position</button><button onClick={()=>{setHistoryOpen(false);setHistoryReason("");setHistoricalPaidAmount("");}} className="min-h-9 rounded-lg border border-sky-300 bg-white px-3 text-[11.5px] font-semibold">Not yet</button></div></div>}</section>}
    ...
    {record.lifecycle === "approved" && <section id="payments" className="rounded-xl border border-stone-200 bg-white p-4"><div className="flex flex-wrap items-end justify-between gap-3"><div><h2 className="text-[14px] font-semibold">Payments</h2><p className="mt-1 text-[12px] text-gray-500">Historical settlement and real payment transactions combine to form the paid position. Real transactions keep their date and method.</p></div></div>
      {record.legacySourceClaimId && record.paymentHistoryKnown && <div className="mt-3 rounded-lg border border-stone-200 bg-stone-50 p-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11.5px] font-medium text-gray-500">Historical amount already paid</p><p className="mt-1 text-[16px] font-semibold tabular-nums text-botanique-charcoal">{money(position?.historicalPaidAmount ?? record.historicalPaidAmount)}</p><p className="mt-1 max-w-3xl text-[11.5px] leading-relaxed text-gray-600">Confirmed from the older Project Cost history without inventing a transaction date or payment method.{record.paymentHistoryNote ? ` ${record.paymentHistoryNote}` : ""}</p>{record.paymentHistoryConfirmedAt && <p className="mt-1 text-[11px] text-gray-500">Confirmed {when(record.paymentHistoryConfirmedAt)} by {profileName(profileMap.get(record.paymentHistoryConfirmedBy))}</p>}</div>{role === "owner" && !correctingHistory && <button type="button" onClick={()=>{setCorrectingHistory(true);setHistoryCorrectionReason("");}} className="min-h-9 rounded-lg border border-stone-300 bg-white px-3 text-[11.5px] font-semibold">Correct historical position</button>}</div>{correctingHistory && <div className="mt-3 border-t border-stone-200 pt-3"><label className="block text-[12px] font-medium text-gray-700">Correction reason<textarea rows={2} value={historyCorrectionReason} onChange={(e)=>setHistoryCorrectionReason(e.target.value)} className="mt-1 w-full rounded-lg border border-stone-300 bg-white px-3 py-2" placeholder="Explain why the historical payment position needs to be checked again."/></label><p className="mt-1.5 text-[11px] text-gray-600">This removes only the historical settlement assertion and returns the record to unconfirmed history. It does not delete or reverse any real payment transaction.</p><div className="mt-2 flex gap-2"><button disabled={working || !historyCorrectionReason.trim()} onClick={correctHistory} className="min-h-9 rounded-lg bg-red-700 px-3 text-[11.5px] font-semibold text-white disabled:opacity-50">Return to unconfirmed</button><button onClick={()=>{setCorrectingHistory(false);setHistoryCorrectionReason("");}} className="min-h-9 rounded-lg border border-stone-300 bg-white px-3 text-[11.5px] font-semibold">Keep position</button></div></div>}</div>}
      {payments.length ? <ul className="mt-3 divide-y divide-stone-100">{payments.map((item)=><li key={item.id} className="py-3"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[12.5px] font-semibold">{item.paymentNumber}</p><p className="mt-0.5 text-[11.5px] text-gray-500">{shortDate(item.paidAt)} · {methodLabels[item.paymentChannel] || item.paymentChannel}{item.paymentReference ? ` · ${item.paymentReference}` : ""}</p>{item.note && <p className="mt-1 text-[12px] text-gray-600">{item.note}</p>}{item.status === "reversed" && <p className="mt-1 text-[11.5px] text-red-700">Reversed: {item.reversalReason}</p>}</div><div className="text-right"><p className={`text-[13px] font-semibold tabular-nums ${item.status === "reversed" ? "text-gray-400 line-through" : ""}`}>{money(item.amount)}</p>{role === "owner" && item.status === "recorded" && <button onClick={()=>{setReverseId(item.id);setReverseReason("");}} className="mt-1 min-h-8 text-[11.5px] font-semibold text-red-700">Reverse</button>}</div></div>{reverseId===item.id && item.status === "recorded" && <div className="mt-2 rounded-lg border border-red-100 bg-red-50 p-3"><label className="text-[12px] font-medium text-red-900">Reversal reason<textarea rows={2} value={reverseReason} onChange={(e)=>setReverseReason(e.target.value)} className="mt-1 w-full rounded-lg border border-red-200 bg-white px-3 py-2 text-botanique-charcoal"/></label><div className="mt-2 flex gap-2"><button disabled={working || !reverseReason.trim()} onClick={()=>reverse(item)} className="min-h-9 rounded-lg bg-red-700 px-3 text-[11.5px] font-semibold text-white disabled:opacity-50">Confirm reversal</button><button onClick={()=>{setReverseId("");setReverseReason("");}} className="min-h-9 rounded-lg border border-stone-300 px-3 text-[11.5px] font-semibold">Keep payment</button></div></div>}</li>)}</ul> : <p className="mt-3 text-[12.5px] text-gray-500">{unknownHistory ? "Payment history has not yet been confirmed for this imported record." : "No dated payment transaction has been recorded yet."}</p>}
      {payable && <form onSubmit={submitPayment} className="mt-4 grid gap-3 rounded-lg bg-stone-50 p-3 sm:grid-cols-2"><div className="sm:col-span-2"><h3 className="text-[12.5px] font-semibold">Settle remaining balance</h3><p className="mt-1 text-[11.5px] text-gray-500">Record the real payment now, using its actual amount, date and method.</p></div><Field label="Amount (KES)"><input type="number" min="0.01" max={position.balanceAmount} step="0.01" value={payment.amount} onChange={(e)=>setPayment({...payment,amount:e.target.value})} className={input}/></Field><Field label="Date"><input type="date" max={today()} value={payment.paidAt} onChange={(e)=>setPayment({...payment,paidAt:e.target.value})} className={input}/></Field><Field label="Method"><select value={payment.paymentChannel} onChange={(e)=>setPayment({...payment,paymentChannel:e.target.value})} className={input}><option value="mpesa">M-Pesa</option><option value="bank_transfer">Bank transfer</option><option value="cash">Cash</option><option value="other">Other</option></select></Field><Field label="Reference (optional)"><input value={payment.paymentReference} onChange={(e)=>setPayment({...payment,paymentReference:e.target.value})} className={input}/></Field><Field label="Note (optional)"><textarea rows={2} value={payment.note} onChange={(e)=>setPayment({...payment,note:e.target.value})} className={input}/></Field><div className="flex items-end justify-end"><button disabled={working || !Number(payment.amount)} className="min-h-10 rounded-lg bg-botanique-green px-4 text-[12.5px] font-semibold text-white disabled:opacity-50">Record payment</button></div></form>}
    </section>}
  </section>;
}
