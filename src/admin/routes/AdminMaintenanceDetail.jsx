...
export default function AdminMaintenanceDetail() {
  ...
  const currentState = currentVisit ? visitWorkState(currentVisit, currentEntry) : "not_scheduled";
  ...
  const cycleStatus = ({ needs_closure: "Needs closure", awaiting_review: "Awaiting review", correction_needed: "Correction needed", draft_field_record: "Draft field record", overdue: "Overdue", due: "Due today", upcoming: "Upcoming", not_scheduled: "No visit scheduled" })[currentState] || "Current";
  ...
  return <section className="space-y-4">
    ...
    <section className="rounded-xl border border-stone-200 bg-white p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><p className="text-[10.5px] font-semibold uppercase tracking-[0.14em] text-gray-500">Current cycle</p><h2 className="mt-1 text-lg font-semibold">{currentVisit ? `${showDate(currentVisit.scheduledDate)} · ${currentVisit.purpose}` : "No visit scheduled"}</h2></div>
        <span className="rounded-full bg-stone-100 px-2.5 py-1 text-[11px] font-semibold text-gray-700">{cycleStatus}</span>
      </div>
      <div className="mt-4 grid gap-3 border-t border-stone-100 pt-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Position label="Field execution" value={currentEntry ? entryLabel(currentEntry.state) : currentVisit ? "Not recorded" : "—"}/>
        <Position label="Arrangement" value={frequencyLabel(relationship.frequency)}/>
        <Position label="Scope" value={relationship.scope}/>
      </div>
      {relationship.status === "active" && canManageMaintenance(role) && <div className="mt-4 flex flex-wrap gap-2 border-t border-stone-100 pt-4">
        ...
        {["awaiting_review", "correction_needed", "draft_field_record"].includes(currentState) && currentEntry && <Link to={`/admin/daily-site-operations/${currentEntry.id}`} className="inline-flex min-h-10 items-center rounded-lg bg-botanique-green px-4 text-[12px] font-semibold text-white">Open site record</Link>}
        ...
        {currentState === "not_scheduled" && <button onClick={() => { setVisitForm({ scheduledDate: today(), purpose: relationship.scope }); setShowVisitForm(true); }} className="min-h-10 rounded-lg bg-botanique-green px-4 text-[12px] font-semibold text-white">Schedule visit</button>}
      </div>}
    </section>

    {cycleForm && <form onSubmit={submitCompletion} className="rounded-xl border border-sky-200 bg-sky-50 p-4">
      <div className="flex items-start justify-between gap-3"><h2 className="text-[14px] font-semibold">Close visit</h2><button type="button" onClick={() => setCycleForm(null)} className="text-[12px] font-semibold text-gray-600">Close</button></div>
