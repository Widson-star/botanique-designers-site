...
describe("Project Costs admin surfaces", () => {
  ...
  it("shows whole-claim Principal decisions, immutable history and stale recovery", async () => {
    ...
    expect(screen.getByRole("button", { name: "Approve cost" })).toBeInTheDocument();
    expect(screen.getByText("Submitted for review")).toBeInTheDocument();
    expect(screen.getByText("Approval and payment are separate facts.")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Approve cost" }));
...
describe("Project Costs possible-duplicate warning", () => {
  ...
  it("warns the Principal when another claim from the same record has an identical line", () => {
    ...
    expect(screen.getByText("Check similar cost")).toBeInTheDocument();
    expect(screen.getByText(/contains an identical line/i)).toBeInTheDocument();
  ...
  it("leaves the Principal's decision entirely intact", () => {
    ...
    expect(screen.getByRole("button", { name: "Approve cost" })).toBeEnabled();
    expect(screen.getByRole("button", { name: "Request correction" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Reject cost" })).toBeInTheDocument();
  });
